...
impl<'a, 'r> FromRequest<'a, 'r> for BasicAuthentication {
	...
	fn from_request(
		request: &'a Request<'r>,
	) -> request::Outcome<BasicAuthentication, String> {
		...
		lazy_static! {
			static ref RE: Regex =
				Regex::new(r"^Basic ([[[:alnum:]]+/=]+)$").unwrap();
		}

		let b64_credentials =
			RE.captures(auth_header).map(|c| c[1].to_string());
		if let Some(credentials) =
			b64_credentials.map_or(None, b64_to_credentials)
		{
