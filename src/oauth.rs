...
use rocket::http::{Cookies, Method, Status};
...
fn grant_post(
    ...
    let session =
        Session::from_cookies(&mut cookies).ok_or(Custom(Status::Unauthorized, "No cookie :("))?;
    let data = form.into_inner();
    let state = AuthState::decode_b64(&data.state).unwrap();
    if data.grant {
...
mod test {
    ...
    fn normal_flow() {
        ...
        let state_regex =
            Regex::new("<input type=\"hidden\" name=\"state\" value=\"([^\"]+)\">").unwrap();
        ...
        let authorization_code =
            token_store.create_token(&String::from(client_id), user, &String::from(redirect_uri));
